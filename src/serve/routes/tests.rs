...
fn register_child_workspace(
    parent_store: &TicketStore,
    child_store: &TicketStore,
) {
    parent_store
        .add_scan_root(ScanRoot {
    ...
    parent_store.scan(true).expect("scan child workspace");
}
...
async fn ancestor_graph_ref_from_child_workspace_is_followable() {
    ...
    register_child_workspace(&parent_store, &child_store);
    parent_store
    ...
    child_store.scan(true).expect("scan child edge");
...
async fn workspace_graph_includes_isolated_local_and_cross_workspace_nodes() {
    ...
    let child_workspace = primary_workspace_name(child_dir.as_path());
    assert!(workspace_payload["workspaces"].as_array().unwrap().iter().any(
        |workspace| workspace["name"] == child_workspace
    ));
    let request = Request::builder()
        .method(Method::GET)
