...
pub fn reapply_workspace_scan_policy(
    store: &TicketStore,
    workspace_root: &Path,
) -> Result<(), ticket_api::error::StorageError> {
...
mod tests {
    ...
    fn reapply_workspace_scan_policy_skips_restricted_descendants() {
        ...
        .expect("write restrictive workspace policy");
        ...
        assert!(
            store
                ...
                .iter()
                .all(|root| root.path != child_store_root.join("tickets")),
            "restrictive policy must not register descendant scan roots"
        );
    }
    #[tokio::test]
